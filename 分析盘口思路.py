...
def parse_fundamentals(match, url):
    ...
    if len(match_group) > 0:
        ...
        match_type = re.findall(r"(.*?)(第|分组|小组|资格|半|决赛|十六|八|季军|外围|排名|附加|升|降|春|秋|16|附加赛|欧会杯资格赛)", match["match_group"])
        match_category = re.findall(
            r"\d+/?\d+(.*?)(第|分组|小组|资格|半|决赛|十六|八|季军|外围|排名|附加|升|降|春|秋|16|附加赛|欧会杯资格赛)",
            match["match_group"])
